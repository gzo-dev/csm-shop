...
const HistoryEditProduct = (props) => {
  ...
  useEffect(() => {
    if (inView === true) {
      (async () => {
        try {
          const result = await get_list_history_edit_product({
            product_id: props?.id,
          });
          setData(result?.data)
        } catch (error) {
  ...
  return (
    <div>
      <Typography className="delete-btn" onClick={() => handleClickOpen()}>
        <FaHistory />
      </Typography>
      <Dialog
        open={open}
...
export default HistoryEditProduct;
